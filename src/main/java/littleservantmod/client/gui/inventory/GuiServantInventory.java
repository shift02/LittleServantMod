...
public class GuiServantInventory extends GuiSideTabContainer {
    ...
    public void renderHealth(int width, int height) {
        ...
        int health = MathHelper.ceil(servant.getHealth());
        boolean highlight = healthUpdateCounter > updateCounter && (healthUpdateCounter - updateCounter) / 3L % 2L == 1L;

        if (health < this.servantHealth && servant.hurtResistantTime > 0) {
            this.lastSystemTime = Minecraft.getSystemTime();
            this.healthUpdateCounter = this.updateCounter + 20;
        } else if (health > this.servantHealth && servant.hurtResistantTime > 0) {
            this.lastSystemTime = Minecraft.getSystemTime();
        ...
        IAttributeInstance attrMaxHealth = servant.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH);
        float healthMax = (float) attrMaxHealth.getAttributeValue();
        float absorb = MathHelper.ceil(servant.getAbsorptionAmount());
        ...
        int regen = -1;
        if (servant.isPotionActive(MobEffects.REGENERATION)) {
        ...
        int MARGIN = 16;
        if (servant.isPotionActive(MobEffects.POISON)) MARGIN += 36;
        else if (servant.isPotionActive(MobEffects.WITHER)) MARGIN += 72;
